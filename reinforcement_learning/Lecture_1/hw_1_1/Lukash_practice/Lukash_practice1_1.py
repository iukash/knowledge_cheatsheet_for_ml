...
class AgentCrossEntropy():
    def __init__(self, action_n, taxi_loc_n, pass_loc_n, dest_loc_n):
            ...
            for pass_loc in range(self.pass_loc_n):
                for dest_loc in range(self.dest_loc_n):
                    if np.sum(new_model[taxi_loc][pass_loc][dest_loc]) > 0:
                        new_model[taxi_loc][pass_loc][dest_loc] /= np.sum(new_model[taxi_loc][pass_loc][dest_loc])
                    else:
