...
class CrossEntropy:  
    ...
    Переменные:     - self.q - квантиль для элитных траекторий ((1 - q)*100 = процент элитных траекторий)     - self.l_laplace - сглаживание по Лапласу (если None - нет сглаживания)     - self.n_env_trajectory - количество траекторий     - self.is_print - печатать информацию на каждой итерации  
    Функции:     - self.fit - запуск процесса обучения     - self.get_trajectories - получение траекторий     - self.policy_improvement - улучшение политики через элитные траектории    """  
