...
class DeepCrossEntropy(nn.Module):
    ...
    def __init__(self, env, q, n_trajectories, n_episode, n_neurons, eps, lr, is_print=True, init_model=None):
        super().__init__()
        self.env = env
        self.q = q
        ...
        self.network = nn.Sequential(nn.Linear(6, n_neurons),
                                     nn.ReLU(),
...
if __name__ == '__main__':
    dce = DeepCrossEntropy(gym.make("Acrobot-v1"), q=0.9, n_trajectories=50, n_episode=200, n_neurons=128, eps=1,
                           lr=0.01, is_print=True, init_model=np.array([0.5, 0, 0.5]))
    dce.fit()
