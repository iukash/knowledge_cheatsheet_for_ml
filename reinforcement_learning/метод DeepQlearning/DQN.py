...
class DQN(nn.Module):
    ...
    def policy_improvement(self, state, action, reward, done, next_state):
        ...
        if len(self.memory) > self.batch_size:
            ...
            targets = rewards + self.gamma * (1 - dones) * torch.max(self.network(next_states), dim=1).values
    ...
    def fit(self, max_len=1000):
        for episode in range(self.n_episode):
            ...
            #print(f'iteration {episode} reward {np.round(total_reward, 3)} eps {np.round(self.epsilon, 3)} ')
            self.mean_total_rewards.append(np.round(total_reward, 3))


if __name__ == '__main__':
    dqn = DQN(env=gym.make('LunarLander-v2'), lr=0.0005, n_episode=500, n_neurons=128, eps_decay=0.95)
    dqn.fit()
